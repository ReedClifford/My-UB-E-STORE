...
$id= $_GET['updateuniid5'];
$sqldisplay2 = "SELECT * from uniformaccesory where id = $id";
$resultdisplay2 = mysqli_query($connect,$sqldisplay2);
$row2 = mysqli_fetch_assoc($resultdisplay2);
$uniformname = $row2['name'];
$uniformprize = $row2['prize'];
$uniformimg = $row2['uniform_img'];
$uniformqty = $row2['quantity'];
...
if (isset($_POST['update5'])) {
    // $bookid = $_POST['id'];
    $uniformname = $_POST['name'];
    $uniformprize = $_POST['prize'];
    $uniformimg = $_FILES['uniform_img'];
    $uniformqty = $_POST['quantity'];


    $uniformimgfilename = $uniformimg['name'];
    $uniformimgerror = $uniformimg['error'];
    $uniformimgfiletemp = $uniformimg['tmp_name'];

    $filename_seperate = explode('.', $uniformimgfilename);
    ...
    if (in_array($file_extension, $extensions)) {
        ...
        $uploadUniformImg = 'images/' . $uniformimgfilename;
        move_uploaded_file($uniformimgfiletemp, $uploadUniformImg);

        $sql = "UPDATE uniformaccesory set id=$id,name='$uniformname',prize='$uniformprize',quantity='$uniformqty',uniform_img='$uploadUniformImg' where id=$id";
        $sql2 = "UPDATE uniforminvent set id=$id,name='$uniformname',prize='$uniformprize',quantity='$uniformqty',uniform_img='$uploadUniformImg' where id=$id";
        ...
        <div class="container-fluid px-4 py-3">
            <div class="row">
                <div class="col col-lg-2 col-md-12 col-sm-12 col-xs-12">
                    <nav class="nav flex-column border border-1 mt-5 nav-pills navbar-light bg-light ">
                        <div class="bg-dark border  py-2 px-3">
                            <h4 class="fs-3 fw-bold text-white-50"><i class="fas fa-user-shield "></i> Admin</h4>
                        </div>
                        <div class="bg-dark border  py-2 px-3">
                        ...
                        <a class="nav-link  border " href="admin-uniform.php#section2">Manage Uniform</a>
                ...
                <div class="col col-lg-10 col-md-12 col-sm-12 col-xs-12 ">
                    ...
                    <h5 class="fw-bolder bg-dark text-white-50 py-3 px-3 fs-4 mt-5">Update</h5>
                    <hr>
                    <form method="post" class="border border-2 py-4 px-4 my-2 mx-2 shadow p-3 mb-5 bg-body rounded" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="name" placeholder="Enter Book Name" value="<?php echo $uniformname ?>">

                        </div>
                        <div class="mb-3">
                            <label for="book_prize" class="form-label">Price</label>
                            <input type="text" class="form-control" name="prize" placeholder="Enter Book Price" value="<?php echo $uniformprize ?>">

                        </div>
                        <div class="mb-3">
                            <label class="form-label">Image</label>
                            <input class="form-control" type="file" name="uniform_img" >
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Quantity</label>
                            <input type="text" class="form-control" name="quantity" placeholder="Enter Quantity" value="<?php echo $uniformqty ?>">

                        </div>
                        <button type="submit" name="update5" class="btn btn-dark">Update</button>
                    </form>
